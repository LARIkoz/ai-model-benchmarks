...
Dead benchmarks are kept in `data/benchmarks.json` for historical context but are never cited in routing recommendations. Planned benchmarks may appear in routing as informational references (indicating which benchmark best measures a task category), but their scores are not used for model ranking until data is collected.
